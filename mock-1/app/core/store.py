...
Deliberately not a database. The point of this repo is the HTTP contract, so
storage is a dict behind a lock with the same *semantics* a real datastore
would give us: unique constraints, optimistic concurrency via a version
counter, and stable ordering for pagination.

Everything here is process-local, so it resets on restart and does not work
across multiple workers. `INTERVIEWER.md` covers what changes when this is
swapped for Postgres.
"""
...
def new_id(prefix: str) -> str:
    """Prefixed opaque IDs (`mon_3f9a...`).

    Prefixes make IDs self-describing in logs and stop a monitor ID from being
    accidentally accepted where an alert ID belongs. They are opaque on
...
class Store:
    ...
    def hit_rate_limit(
        ...
        cheap but allow a burst of up to 2x the limit across a boundary; see
        `INTERVIEWER.md` for the sliding-window/token-bucket discussion.
        """
        with self._lock:
    ...
    def reset(self) -> None:
        """Wipe all state. Used by tests and the /admin/reset dev endpoint."""
        with self._lock:
