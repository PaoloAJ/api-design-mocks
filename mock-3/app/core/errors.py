...
exception — leaves the process through `register_error_handlers`, so a client
parses one predictable body instead of guessing between Flask's HTML 404 page
and our own JSON.
"""
...
class APIError(Exception):
    ...
    them exactly which thing went wrong.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Field-level detail, e.g. [{"field": "body", "message": "required"}].
...
class ForbiddenError(APIError):
    ...
    Distinct from the 404 used for another tenant's resources: here the caller
    is legitimately ours and the resource is legitimately theirs, so confirming
    it exists leaks nothing.
    """
