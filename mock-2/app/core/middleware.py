...
# hardcoded list. Same two demo tenants as mock-1, on purpose.
VALID_API_KEYS = {
    "dd-demo-key-alpha": "org_alpha",
    "dd-demo-key-beta": "org_beta",
}
...
def register_middleware(app) -> None:
    ...
    def authenticate():
        if request.path in PUBLIC_PATHS or request.method == "OPTIONS":
            return None
        api_key = request.headers.get("DD-API-KEY") or _bearer_token()
        if not api_key:
            raise UnauthorizedError(
                "Missing credentials. Send a DD-API-KEY header.",
                code="missing_api_key",
            )
        org_id = VALID_API_KEYS.get(api_key)
        if org_id is None:
