...
def new_id(prefix: str) -> str:
    """Prefixed opaque IDs (`pay_3f9a...`).

    Prefixes make IDs self-describing in logs and stop a payment ID from being
    accidentally accepted where a refund ID belongs. They are opaque on
...
class Store:
    ...
    def reset(self) -> None:
        """Wipe all state. Used by tests and to reseed cleanly on restart."""
        with self._lock:
