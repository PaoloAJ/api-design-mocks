...
Offset pagination (`?page=3`) drifts: if rows are inserted while a client
walks the list, it sees duplicates or misses rows, and deep offsets force the
database to count past everything it skips. Cursors encode "where I stopped"
as a sort-key value, so the next page is an index seek and concurrent inserts
