"""Cross-cutting request concerns: auth, rate limiting, request IDs, timing.
...
# hardcoded list.
VALID_API_KEYS = {
    "sk_test_alpha_demo": "merchant_alpha",
    "sk_test_beta_demo": "merchant_beta",
}
...
def register_middleware(app) -> None:
    ...
    def assign_request_id():
        ...
        merchant paste an ID into a support ticket and have us find the exact
        log line.
        """
    ...
    def authenticate():
        if request.path in PUBLIC_PATHS or request.method == "OPTIONS":
            return None
        api_key = request.headers.get("X-Api-Key") or _bearer_token()
        if not api_key:
            raise UnauthorizedError(
                "Missing credentials. Send an X-Api-Key header.",
                code="missing_api_key",
            )
        merchant_id = VALID_API_KEYS.get(api_key)
