...
def test_default_limit_and_has_more(client, auth):
    ...
    body = client.get("/v1/payments", headers=auth).get_json()
...
def test_limit_is_clamped_not_rejected(client, auth):
    _make(client, auth, 3)
    body = client.get("/v1/payments?limit=100000", headers=auth).get_json()
    assert body["pagination"]["limit"] == MAX_LIMIT


def test_zero_limit_is_400(client, auth):
    assert client.get("/v1/payments?limit=0", headers=auth).status_code == 400


def test_non_integer_limit_is_400(client, auth):
    assert client.get("/v1/payments?limit=abc", headers=auth).status_code == 400


def test_malformed_cursor_is_400(client, auth):
    response = client.get("/v1/payments?cursor=!!!notbase64!!!", headers=auth)
    assert response.status_code == 400


def test_cursor_roundtrip():
    payload = {"created_at": "2026-01-01T00:00:00.000Z", "id": "pay_x"}
